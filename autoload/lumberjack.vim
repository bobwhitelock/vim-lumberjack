...
function! lumberjack#OperatorPrintBelow(motion_wise) abort
  call s:handle_operator_print(a:motion_wise, 'o')
endfunction

function! lumberjack#OperatorPrintAbove(motion_wise) abort
  call s:handle_operator_print(a:motion_wise, 'O')
endfunction

function! s:handle_operator_print(motion_wise, enter_insert_char) abort
...
function! s:yank_last_motion(motion_wise) abort
...
function! s:template_print_string(print_string, value) abort
...
function! s:include_random_token() abort
...
" technique.
function! s:template_string(string, values_map) abort
...
function! s:random_string(length) abort
...
function! s:pseudo_random_integer_between(min, max) abort
